...
DrawingPageLGN::DrawingPageLGN()
{
	...
	designPressurePattern.assign(LR"(-?\d(\.\d{1,4})?( \/ (FV)?(\d\.\d{2})?)?|atm\.|hydr\.|to\/до \d)");
	...
	lineNumberPattern.assign(LR"(\d{3}-[A-Z0-9]{2,3}-\d{4}-[A-Z0-9]{9,11}|Protective duct drain\/Слив защитного желоба №\d{2}_\d{3}_\d{2})");
	stressCalculationPattern.assign(LR"(NO\/НЕТ|YES\/ДА)");
	isometricDrawingPattern.assign(LR"(\d{3}-[A-Z0-9]{2,4}-\d{4}-[A-Z0-9]{9,11}|Protective duct drain\/Слив защитного желоба №\d{2}_\d{2,3}_\d{2})");
