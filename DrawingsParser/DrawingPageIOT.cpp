...
DrawingPageIOT::DrawingPageIOT(bool inputCheckOff) : DrawingPage(inputCheckOff)
{
	...
	operatingPressurePattern.assign(LR"(\d(\.\d{1,3})?(\w{2}-\d{2})?(…\d(\.\d{1,2})?)?)");
	...
	paintingSystemPattern.assign(LR"([A-ZА-Я]\d( ?(→|->) ?[A-ZА-Я]\d)?(,[A-ZА-Я]\d ?(→|->) ?[A-ZА-Я]\d)?|NO\/НЕТ)");
	...
	schemeNumberPattern.assign(LR"(GCC-(T|I)OT-DDD-\d{5}-\d{2}-\d{4}-[A-ZА-Я]{2,4}\d?-\w{3}-\d{5}(-\d+)?|NO\/НЕТ)");
