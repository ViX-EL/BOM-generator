...
void TableWriter::writePagesFields()
{
	...
	for (size_t drawingIdx = 0; drawingIdx < drawings->size(); drawingIdx++)
	{
		for (size_t pageIdx = 0; pageIdx < (*drawings)[drawingIdx].pages.size(); pageIdx++)
		{
			...
			wxLogMessage(L"[Запись] Конец записи листа %s файла %s", currentDrawingPage->getCurrentPage(), currentDrawingPage->getFileName());
			//printer->printText(L"Конец записи листа " + std::to_wstring(currentDrawingPage->getCurrentPage()) + L" файла " + currentDrawingPage->getFileName(), L"[Запись]");
		}
	}
}
